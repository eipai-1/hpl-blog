...
public class CommonPageParam {
    ...
    public static CommonPageParam newInstance(){
        return newInstance(DEFAULT_PAGE_NUM,DEFAULT_PAGE_SIZE);
    }

    public static CommonPageParam newInstance(Integer pageNum, Integer pageSize){
     ...
     */
    public static CommonPageParam newInstance(Long pageNum, Long pageSize){
        // 检查并修正页码，确保其为有效值
        ...
        final CommonPageParam commonPageParam = new CommonPageParam();
        // 设置实例的页码和每页大小属性
        commonPageParam.pageNum=pageNum;
        commonPageParam.pageSize=pageSize;
        // 计算查询的偏移量，用于数据库查询的偏移定位
        commonPageParam.offset=(pageNum-1)*pageSize;
        // 设置查询的限制，即每页的大小
        commonPageParam.limit=pageSize;
     ...
     *
     * @param commonPageParam 分页参数对象，包含偏移量和每页记录数。
